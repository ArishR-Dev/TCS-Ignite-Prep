    ...
    >
      ...
      {slide.isMandatoryChecklist ? (
        ...
        <div className={`flex-1 flex flex-col justify-center my-2 sm:my-6 py-3.5 sm:py-8 ${spacing.cardPadding} rounded-xl bg-slate-900/60 border border-slate-800/60 backdrop-blur-sm relative z-10`}>
          ...
          <p className={`${isMobile ? 'text-xs sm:text-sm' : 'text-lg sm:text-xl'} text-slate-200 leading-relaxed max-w-3xl mb-3 sm:mb-8`}>
            {slide.content.paragraphs?.[0]}
          </p>

          {slide.content.keyNotes && slide.content.keyNotes.length > 0 && (
            <div className="grid sm:grid-cols-2 gap-2 sm:gap-4 mt-2">
              {slide.content.keyNotes.map((note, idx) => (
                <div key={idx} className={`flex items-start gap-2 ${spacing.cardPadding} rounded-lg bg-slate-800/40 border border-slate-700/50`}>
                  <div className="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-cyan-400 mt-1.5 flex-shrink-0" />
                  <span className={`${typography.body} text-slate-300 font-medium`}>{note}</span>
                </div>
              ))}
            </div>
          )}
        ...
        <div className={`flex-1 ${spacing.sectionGap} relative z-10 text-slate-200`}>
          ...
          {slide.content.paragraphs && slide.content.paragraphs.length > 0 && (
            <div className={`space-y-1.5 sm:space-y-2 ${typography.body} text-slate-200`}>
              {slide.content.paragraphs.map((p, idx) => (
                <p key={idx} className={p.startsWith('•') || p.startsWith('1.') || p.startsWith('2.') ? 'font-medium pl-1' : ''}>
                  {p}
                </p>
              ))}
            </div>
          ...
          {slide.content.diagram && (
            <div className={`${spacing.cardPadding} rounded-xl bg-slate-950/80 border border-slate-800 ${typography.diagram} text-cyan-300 overflow-x-auto whitespace-pre shadow-inner touch-pan-x overscroll-x-contain`}>
              {slide.content.diagram}
            </div>
          )}
            ...
            <div className="space-y-2.5 sm:space-y-4">
              {slide.content.codeBlocks.map((cb, idx) => (
                <div key={idx} className="rounded-xl overflow-hidden border border-slate-800 bg-[#090d16] shadow-lg">
                  ...
                  <pre className={`${spacing.cardPadding} ${typography.code} text-sky-200 overflow-x-auto bg-[#060a12] touch-pan-x overscroll-x-contain`}>
                    <code>{cb.code}</code>
                  </pre>
                  {cb.output && (
                    <div className={`${spacing.cardPadding} py-1.5 sm:py-2.5 bg-slate-900/60 border-t border-slate-800/80 font-mono text-[10px] sm:text-xs text-slate-300 overflow-x-auto`}>
                      <span className="text-slate-400 block mb-0.5 font-semibold uppercase tracking-wider text-[9px] sm:text-[10px]">Output / Note:</span>
                      ...
                      <tbody className="divide-y divide-slate-800/80 bg-slate-950/40">
                        {tbl.rows.map((row, rIdx) => (
                          <tr key={rIdx} className={rIdx % 2 === 0 ? 'bg-slate-900/20' : 'bg-slate-900/50'}>
                            {row.map((cell, cIdx) => (
                              <td key={cIdx} className="px-2 sm:px-4 py-1.5 sm:py-2.5 text-slate-300 leading-relaxed">
                                {cell}
                              </td>
                            ))}
                          </tr>
          ...
          {/* Callouts (Interview Answers, Warnings, Memory Tricks) */}
          {slide.content.callouts && slide.content.callouts.length > 0 && (
            <div className="space-y-2 sm:space-y-3">
              {slide.content.callouts.map((callout, idx) => {
                let borderClass = 'border-cyan-500/40 bg-cyan-950/20 text-cyan-100';
                let badgeClass = 'bg-cyan-500/20 text-cyan-300';
                let Icon = Sparkles;

                if (callout.type === 'interview') {
                  borderClass = 'border-emerald-500/40 bg-emerald-950/20 text-emerald-100';
                  badgeClass = 'bg-emerald-500/20 text-emerald-300';
                  Icon = HelpCircle;
                } else if (callout.type === 'warning') {
                  borderClass = 'border-amber-500/40 bg-amber-950/20 text-amber-100';
                  badgeClass = 'bg-amber-500/20 text-amber-300';
                  Icon = AlertTriangle;
                } else if (callout.type === 'remember') {
                  borderClass = 'border-blue-500/40 bg-blue-950/20 text-blue-100';
                  badgeClass = 'bg-blue-500/20 text-blue-300';
                  Icon = Sparkles;
                }

                return (
                  <div key={idx} className={`${spacing.cardPadding} rounded-xl border ${borderClass} shadow-sm backdrop-blur-sm`}>
                    <div className="flex items-center gap-1.5 mb-1 sm:mb-1.5">
                      <span className={`px-1.5 py-0.5 rounded ${typography.badge} font-semibold uppercase tracking-wider flex items-center gap-1 ${badgeClass}`}>
                        <Icon className="w-3 h-3 flex-shrink-0" />
                        <span className="truncate">{callout.label}</span>
                      </span>
                    </div>
                    <div className={`${typography.callout} text-slate-200 whitespace-pre-wrap`}>
                      {callout.content}
                    </div>
                  </div>
                );
